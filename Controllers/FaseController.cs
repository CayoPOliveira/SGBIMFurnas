...
public class FaseController : ControllerBase
{
    ...
    [HttpGet]
    public IEnumerable<FaseReadDto> RecuperaFases([FromQuery] int skip = 0, [FromQuery] int take = 15)
    {
        return _mapper.Map<List<FaseReadDto>>(_context.Fases.Where(fase => fase.Valid).Skip(skip).Take(take).ToList());
    }
    ...
    public IActionResult RecuperaFaseComId(int id)
    {
        ...
        return faseEncontrada == null ? NotFound() : Ok(_mapper.Map<FaseWithEtapaReadDto>(faseEncontrada));
    }

    [HttpGet("{id}/Seguintes")]
    public IActionResult RecuperaFaseSeguintesComId(int id)
    {
        Fase faseEncontrada = _context.Fases.FirstOrDefault(fase => fase.Id == id && fase.Valid);
    ...
    public IActionResult RecuperaFaseAnterioresComId(int id)
    {
        Fase faseEncontrada = _context.Fases.FirstOrDefault(fase => fase.Id == id && fase.Valid);
        return faseEncontrada == null ? NotFound() : Ok(_mapper.Map<FaseWithAnterioresReadDto>(faseEncontrada));
    }

    [HttpGet("{id}/Anteriores/Seguintes")]
    public IActionResult RecuperaFaseAnterioresSeguintesComId(int id)
    {
        Fase faseEncontrada = _context.Fases.FirstOrDefault(fase => fase.Id == id && fase.Valid);
        return faseEncontrada == null ? NotFound() : Ok(_mapper.Map<FaseWithEtapaAnteriorSeguinteReadDto>(faseEncontrada));
    }
